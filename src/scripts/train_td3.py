...
def train_multi(args: argparse.Namespace) -> None:
    ...
    bytes_per_transition = (2 * env.observation_space.shape[0] + env.action_space.shape[0] + 3) * np.dtype(np.float32).itemsize
    ...
    for label, paths in splits.items():
        td3 = evaluate_paths(model, paths, args.seed, env)
